...
import { MoreHorizontal, Edit, Trash, Columns, ChevronDown } from "lucide-react"
        ...
        <div className="min-w-full inline-block align-middle max-w-1">
          <div className="overflow-hidden">
            <Table>
              <TableHeader>
                <TableRow>
                  {headers.filter(header => visibleColumns.includes(header.key)).map((header) => (
              ...
              <TableBody>
                {currentItems.map((item, index) => (
                  <TableRow key={index}>
                    {headers.filter(header => visibleColumns.includes(header.key)).map((header) => (
                      <TableCell key={header.key} className="whitespace-nowrap">{item[header.key]}</TableCell>
                    ))}
