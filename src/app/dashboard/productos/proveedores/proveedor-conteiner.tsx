...
function ProveedorContainer({ data, headers }: ProveedorContainerProps) {
    ...
    const handleDelete = async () => {
        ...
        const data = await deleteProveedor(itemDeleted?.id);
        if (!data) {
    ...
    return (
        <>
            ...
            <div className="flex justify-between gap-4">
                <DialogForm setItemEditing={setItemEditing} open={open} setOpen={setOpen} textButton="Agregar nuevo proveedor" titleDialog="REGISTRO DE PROVEEDOR" descriptionDialog="Agrega un nuevo proveedor">
                    <FormSupplier
                        setOpen={setOpen}
                        data={{
